  ...
  constructor(props) {
    ...
    let curTestObj =
      this.props.tests && this.props.tests.curTest
        ...
        : null;
    let curQuestion =
      curTestObj && curQuestionId && curTestObj.questions
        ? curTestObj.questions[curQuestionId]
        : null;

    this.state = {
      curQuestionResponse:
        curQuestion && curQuestion.response ? curQuestion.response : '',
    };
  ...
  render() {
    ...
    let curQuestion =
      curTestObj && curQuestionId && curTestObj.questions
        ? curTestObj.questions[curQuestionId]
        : null;
    let curQuestionStatus =
      curQuestion && curQuestion.status ? curQuestion.status : 'not visited';
    let curQuestionResponse =
      curQuestion && curQuestion.response ? curQuestion.response : '';
    let curQuestionContent =
      curQuestion && curQuestion.content ? curQuestion.content : '';
    let curQuestionChild =
      curQuestion && curQuestion.child_content
        ? curQuestion.child_content
        : null;
        ...
        {curTestStatus === 'active' && (
          <React.Fragment>
            <div className={styles.testSummaryHeading}>
              <h1>
                Question {curTestObj.questions[curQuestionId].sort_order + 1}
              </h1>
          ...
          <div>
            ...
            <div>
              <span className={styles.linkButton}>
                <NavLink to="/tests/current">Back</NavLink>
              </span>
            </div>
          </div>
        )}
