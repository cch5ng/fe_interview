...
class Tests extends Component {
  ...
  componentDidMount() {
    let email =
      this.props.auth && this.props.auth.email ? this.props.auth.email : null;
    if (email) {
  ...
  render() {
    ...
    return (
      <div className={styles.testsContainer}>
        ...
        {testError && <div className={styles.errorMessage}>{testError}</div>}

        {testsAr.map(test => {
          const missedQuestions = test.questions
            ? this.getQuestionsMissedCount(test.questions)
            : null;
          const completedQuestionsCount = test.questions
            ? test.questions.length - missedQuestions
            : null;
          ...
          return (
            <div className={styles.test} key={test.id} data-testid="test-item">
              <Link to={testSummaryUrl}>
                <ul>
                  <li
                    className={[styles.testsListDetail, styles.testName].join(
                      ' '
                    )}>
                    {test.name}
                  </li>
                  <li className={styles.testsListDetail}>
                    Created on {getPrettyDate(test.date_taken)}
                  </li>
                  <li className={styles.testsListDetail}>
                    {test.questions ? test.questions.length : null} questions
                  </li>
                  <li className={styles.testsListDetail}>
                    {this.prettyMissedQuestionsStr(missedQuestions)}
                  </li>
                  <li className={styles.testsListDetail}>
                    Completed in{' '}
                    {getPrettyTime(test.time_total - test.time_remaining)} /{' '}
                    {getPrettyTime(test.time_total)}
                  </li>
                </ul>
              </Link>
            </div>
