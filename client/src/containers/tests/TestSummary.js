...
class TestSummary extends Component {
  constructor(props) {
    ...
    this.getQuestionStatusIconClass = this.getQuestionStatusIconClass.bind(
      this
    );
  ...
  startTest(ev) {
    ...
    let curTest =
      this.props.tests && this.props.tests.curTest
        ? this.props.tests.curTest
        : null;
    let curTestTimeTotal =
      curTest && curTest.time_total ? curTest.time_total : 0;
    let randomQuestAr =
      this.props.tests &&
      this.props.tests.curTest &&
      this.props.tests.curTest.questions
        ? dictToRandomAr(this.props.tests.curTest.questions)
        : [];
    let firstQuestionUrl = randomQuestAr.length
      ? `/tests/question/${randomQuestAr[0].id}`
      : null;
  ...
  renderIcon(status) {
    if (status === 'not_visited') {
      return <EmptyBox name="unchecked box" height="24px" width="24px" />;
    }
    if (status === 'skipped') {
      return <Warning name="warning" height="24px" width="24px" />;
    }
    if (status === 'completed') {
      return <CheckedBox name="checked box" height="24px" width="24px" />;
    }
  }
  ...
  render() {
    let curTestObj =
      this.props.tests && this.props.tests.curTest
        ...
        : null;
    let status =
      curTestObj && curTestObj.status ? curTestObj.status : 'initialized';
    let randomQuestAr =
      this.props.tests &&
      this.props.tests.curTest &&
      this.props.tests.curTest.questions
    ...
    let firstQuestionUrl = randomQuestAr.length
      ? `/tests/question/${randomQuestAr[0].id}`
      : null;
    let prettyTotalTime = curTestObj
      ? getPrettyTime(curTestObj.time_total)
      : '';
    // TODO is timeRemaining used?
    let timeRemaining =
      curTestObj && curTestObj.time_remaining
        ? curTestObj.time_remaining
        : null;

    let timeTaken = curTestObj ? curTestObj.time_total - timeRemaining : null;
    let curTestQuestionsCount =
      curTestObj && curTestObj.questions
        ? Object.keys(curTestObj.questions).length
        : 0;
      ...
      <div className={styles.testSummaryContainer}>
          ...
          <div className={styles.testSummaryHeading}>
            ...
            {this.props.remainingTime > 0 && (
              <div
                className={
                  displayAlarm
                    ? [styles.countdownDisplay, styles.countdownAlarm].join(' ')
          ...
          randomQuestAr.map(question => {
            ...
            return (
              <div key={displayOrder} className={styles.question}>
                Q{displayOrder} {this.renderIcon(question.status)}{' '}
                {formatQuestion(question.content, question.child_content)}
              </div>
            );
          })}
        ...
        {curTestObj && status === 'completed' && (
          <div className={styles.testSummary} data-testid="test_summary">
            <h2>
              <span className={styles.bold}>Name</span> {curTestObj.name}
            </h2>
            <p>
              <span className={styles.bold}>Time used</span>{' '}
              {getPrettyTime(timeTaken)} / {prettyTotalTime}
            </p>
            <p>
              <span className={styles.bold}>Skipped Questions</span>{' '}
              {this.getQuestionsCountByStatus().skipped} /{' '}
              {curTestQuestionsCount}
            </p>
            <p>
              <span className={styles.bold}>Completed Questions</span>{' '}
              {this.getQuestionsCountByStatus().completed} /{' '}
              {curTestQuestionsCount}
            </p>
          </div>
        )}
            ...
            return (
              <div key={displayOrder} className={styles.question}>
                ...
                <span className={styles.linkButton}>
                  <NavLink to={curQuestionUrl}>Go</NavLink>
                </span>
              </div>
            );
          })}
        {status === 'initialized' && (
          <button onClick={this.startTest} className={styles.btnTestSummary}>
            Start
          </button>
        )}
        {status === 'active' && (
          <button onClick={this.submitTest} className={styles.btnTestSummary}>
            Submit
          </button>
        )}
        {status === 'completed' && (
          <div
            className={[styles.linkButton, styles.btnTestSummaryBack].join(
              ' '
            )}>
